...
func DEXTest(ctx *testframework.TestFrameworkContext) bool {
	...
	txHash, err = ctx.Ont.NeoVM.InvokeNeoVMContract(ctx.GetGasPrice(), ctx.GetGasLimit(),
		account2,
		codeAddress,
		[]interface{}{"addOrder", []interface{}{account2.Address[:],1,1500000000,1,2*priceMultiple }})
	if err != nil {
		ctx.LogError("Dice invest error: %s", err)
	}
	...
	txHash, err = ctx.Ont.NeoVM.InvokeNeoVMContract(ctx.GetGasPrice(), ctx.GetGasLimit(),
		signer,
		codeAddress,
		[]interface{}{"matchOrder", []interface{}{buyorderid,sellorderid,100*priceMultiple,2*priceMultiple,1}})
	if err != nil {
		ctx.LogError("Dice invest error: %s", err)
	}
