...
char * selfAddr(){
    return  ONT_Runtime_AddressToBase58(ONT_Runtime_GetExecutingAddress());
}

char * callerAddr(){
    return  ONT_Runtime_AddressToBase58(ONT_Runtime_GetCallingAddress());
}

char * callerAddr(){
    return  ONT_Runtime_AddressToBase58(ONT_Runtime_GetEntryAddress());
}

char* invoke(char * method,char * args){
    if (strcmp(method,"selfAddr") == 0){
        return selfAddr();
    }
    if (strcmp(method,"callerAddr") == 0){
        return callerAddr();
    }
    if (strcmp(method,"entryAddr") == 0){
        return entryAddr();
    }
    return "FALSE";
}