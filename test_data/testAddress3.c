...
char * ONT_Runtime_AddressToBase58(char *addr);
char * ONT_Runtime_AddressToHex(char *addr);
...
char * contractaddr = "ARmza3GdeoHKY7Dk5X7KGqT4EabKoE2of7";

char * selfAddr(){
        return ONT_CallContract(contractaddr,"callselfAddr",null);

}

char * callerAddr(){
        return ONT_CallContract(contractaddr,"callcallerAddr",null);
}

char * entryAddr(){
        return ONT_CallContract(contractaddr,"callentryAddr",null);
}


char* invoke(char * method,char * args){
    if (strcmp(method,"callselfAddr") == 0){
        return selfAddr();
    }
    if (strcmp(method,"callcallerAddr") == 0){
        return callerAddr();
    }
    if (strcmp(method,"callentryAddr") == 0){
        return entryAddr();
    }
    return "FALSE";
}